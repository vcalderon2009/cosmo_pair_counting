...
PACKAGENAME = metadata.get('package_name', 'cosmo_pair_counting')
DESCRIPTION = metadata.get('description', 'Package for optimized pair counting')
AUTHOR = metadata.get('author', 'Victor Calderon')
AUTHOR_EMAIL = metadata.get('author_email', '')
LICENSE = metadata.get('license', 'unknown')
URL = metadata.get('url', 'https://github.com/vcalderon2009/cosmo_pair_counting')
__minimum_python_version__ = metadata.get("minimum_python_version", "3.6")

# Enforce Python version check - this is the same check as in __init__.py but
# this one has to happen before importing ah_bootstrap.
if sys.version_info < tuple((int(val) for val in __minimum_python_version__.split('.'))):
    sys.stderr.write("ERROR: cosmo_pair_counting requires Python {} or later\n".format(__minimum_python_version__))
    sys.exit(1)
